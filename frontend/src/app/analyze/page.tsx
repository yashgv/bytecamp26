...
export default function AnalyzePage() {
  ...
  const handleAnalyze = async (url?: string | any) => {
    const targetUrl = typeof url === 'string' ? url : repoUrl;
    ...
    try {
      // You may want to configure NEXT_PUBLIC_API_URL in production,
      // but assuming localhost:8000 for local development based on Python backend
      const response = await fetch("http://localhost:8000/github/analyze", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ github_url: targetUrl.trim() }),
      });
      ...
      router.push(`/scanning?repo_id=${data.repo_id}&url=${encodeURIComponent(targetUrl.trim())}`);
    } catch (err: any) {
  ...
  const handleImport = async (url: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || "Failed to import repository");
      }

      const data = await response.json();

      router.push(`/scanning?repo_id=${data.repo_id}&url=${encodeURIComponent(url.trim())}`);
    } catch (err: any) {
      setError(err.message || "An unexpected error occurred");
    } finally {
      setIsLoading(false);
                      ...
                      {loadingRepos ? (
                        ...
                        filteredRepos.map(repo => (
                          <div key={repo.id} className={styles.repoItem}>
                            ...
                            <button className={styles.importRepoBtn} onClick={() => handleImport(repo.html_url)} disabled={isLoading}>
                              Import
                            </button>
                          </div>
                        ))
